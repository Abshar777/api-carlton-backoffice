...
class IncomeExpenseCreate(BaseModel):
    ...
    vendor_supplier_id: Optional[str] = (
        None  # If linked to a service supplier (rent, utilities)
    )
...
class LoanCreate(BaseModel):
    ...
    treasury_account_id: Optional[str] = (
        None  # Source treasury account (optional if disburse_from_vendor_id is provided)
    )
...
class LoanRepaymentCreate(BaseModel):
    ...
    currency: str = "USD"
    treasury_account_id: Optional[str] = (
        None  # Where repayment goes (optional if credit_to_vendor_id provided)
    )
    credit_to_vendor_id: Optional[str] = (
        None  # Credit to vendor/exchanger instead of treasury
    )
    ...
    exchange_rate: Optional[float] = (
        None  # Custom exchange rate (payment currency -> loan currency)
    )
    amount_in_loan_currency: Optional[float] = (
        None  # Pre-calculated equivalent in loan currency
    )
...
async def require_accountant_or_admin(user: dict = Depends(get_current_user)) -> dict:
    if user.get("role") not in [UserRole.ADMIN, UserRole.ACCOUNTANT]:
        raise HTTPException(
            status_code=403, detail="Accountant or Admin access required"
        )
    return user
...
async def initialize_default_roles():
    ...
    for role_data in DEFAULT_ROLES:
        existing = await db.roles.find_one(
            {"role_id": role_data["role_id"]}, {"_id": 0}
        )
        if not existing:
            now = datetime.now(timezone.utc)
...
async def verify_otp(request: Request, data: dict = Body(...)):
    ...
    if not otp_record:
        raise HTTPException(
            status_code=401, detail="No verification code found. Please login again."
        )
    ...
    if datetime.now(timezone.utc) > expires_at:
        await db.otp_codes.delete_many({"user_id": user["user_id"]})
        raise HTTPException(
            status_code=401, detail="Verification code expired. Please login again."
        )

    if otp_record.get("attempts", 0) >= 3:
        await db.otp_codes.delete_many({"user_id": user["user_id"]})
        raise HTTPException(
            status_code=401, detail="Too many attempts. Please login again."
        )
    ...
    if otp_record["code"] != otp_code:
        ...
        raise HTTPException(
            status_code=401, detail=f"Invalid code. {remaining} attempts remaining."
        )
...
async def mfa_setup(request: Request, data: dict = Body(default={})):
    ...
    otpauth = pyotp.TOTP(secret).provisioning_uri(
        name=user["email"], issuer_name="Carlton"
    )
...
async def change_password(
    ...
    if not current_password or not new_password:
        raise HTTPException(
            status_code=400, detail="Current and new passwords are required"
        )
...
async def get_user_security_status(user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "session_timeout_hours": (
            settings.get("session_timeout_hours", 2) if settings else 2
        ),
    }
...
async def reset_password(data: dict = Body(...)):
    ...
    if not email or not otp_code or not new_password:
        raise HTTPException(
            status_code=400, detail="Email, code, and new password are required"
        )
    if len(new_password) < 6:
        raise HTTPException(
    ...
    if not reset_record:
        raise HTTPException(
            status_code=401, detail="No reset code found. Please request again."
        )
    ...
    if datetime.now(timezone.utc) > expires_at:
        await db.password_resets.delete_many({"email": email})
        raise HTTPException(
            status_code=401, detail="Reset code expired. Please request again."
        )

    if reset_record.get("attempts", 0) >= 5:
        await db.password_resets.delete_many({"email": email})
        raise HTTPException(
            status_code=401, detail="Too many attempts. Please request again."
        )
...
async def get_security_settings(
    user: dict = Depends(require_permission(Modules.SETTINGS, Actions.VIEW))
):
...
async def update_security_settings(
    ...
    await db.app_settings.update_one(
        {"setting_type": "security"}, {"$set": updates}, upsert=True
    )
...
async def get_users(
    user: dict = Depends(require_permission(Modules.USERS, Actions.VIEW))
):
...
async def create_user(
    ...
    user: dict = Depends(require_permission(Modules.USERS, Actions.CREATE)),
):
...
async def delete_user(
    request: Request,
    user_id: str,
    user: dict = Depends(require_permission(Modules.USERS, Actions.DELETE)),
):
...
async def get_client(
    ...
    deposits = next(
        (x for x in tx_summary if x["_id"] == "deposit"),
        {"total_amount": 0, "count": 0},
    )
    withdrawals = next(
        (x for x in tx_summary if x["_id"] == "withdrawal"),
        {"total_amount": 0, "count": 0},
    )
    ...
    client["net_balance"] = deposits.get("total_amount", 0) - withdrawals.get(
        "total_amount", 0
    )
    ...
    recent_txs = (
        await db.transactions.find({"client_id": client_id}, {"_id": 0})
        .sort("created_at", -1)
        .limit(10)
        .to_list(10)
    )
...
async def delete_client(
    request: Request,
    client_id: str,
    user: dict = Depends(require_permission(Modules.CLIENTS, Actions.DELETE)),
):
...
async def bulk_upload_clients(
    ...
    for idx, row in enumerate(data_rows):
        try:
            ...
            email = (
                str(row[col_map.get("email", -1)] or "").strip().lower()
                if "email" in col_map
                else ""
            )
            phone = (
                str(row[col_map.get("phone", -1)] or "").strip()
                if "phone" in col_map
                else ""
            )
            country = (
                str(row[col_map.get("country", -1)] or "").strip()
                if "country" in col_map
                else ""
            )
            crm_id = (
                str(row[col_map.get("crm_id", -1)] or "").strip()
                if "crm_id" in col_map
                else ""
            )
    ...
    await log_activity(
        request, user, "create", "clients", f"Bulk uploaded {created} clients"
    )
...
async def get_client_bank_accounts(
    client_id: str,
    user: dict = Depends(require_permission(Modules.CLIENTS, Actions.VIEW)),
):
    """Get all saved bank accounts for a client"""
    accounts = await db.client_bank_accounts.find(
...
async def create_client_bank_account(
    ...
    return await db.client_bank_accounts.find_one(
        {"bank_account_id": bank_account_id}, {"_id": 0}
    )
...
async def update_client_bank_account(
    ...
    result = await db.client_bank_accounts.update_one(
        {"bank_account_id": bank_account_id, "client_id": client_id}, {"$set": updates}
    )
    if result.matched_count == 0:
...
async def delete_client_bank_account(
    request: Request,
    client_id: str,
    bank_account_id: str,
    user: dict = Depends(require_permission(Modules.CLIENTS, Actions.DELETE)),
    ...
    result = await db.client_bank_accounts.delete_one(
        {"bank_account_id": bank_account_id, "client_id": client_id}
    )
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Bank account not found")
    await log_activity(
        request, user, "delete", "clients", "Deleted client bank account"
    )
...
async def get_treasury_accounts(
    ...
    accounts = (
        await db.treasury_accounts.find(query, {"_id": 0})
        .sort("created_at", -1)
        .skip(skip)
        .limit(page_size)
        .to_list(page_size)
    ...
    fx_settings = await db.app_settings.find_one(
...
async def create_treasury_account(
    ...
    request: Request,
    user: dict = Depends(require_permission(Modules.TREASURY, Actions.CREATE)),
):
    ...
    await log_activity(
        ...
        "create",
        "treasury",
        f"Created treasury account: {account_data.account_name}",
...
async def update_treasury_account(
    request: Request,
    account_id: str,
    update_data: TreasuryAccountUpdate,
    user: dict = Depends(require_permission(Modules.TREASURY, Actions.EDIT)),
):
    ...
    result = await db.treasury_accounts.update_one(
...
async def delete_treasury_account(
    request: Request,
    account_id: str,
    user: dict = Depends(require_permission(Modules.TREASURY, Actions.DELETE)),
):
...
async def get_lp_accounts(
    ...
    skip = (page - 1) * page_size
    accounts = (
        await db.lp_accounts.find(query, {"_id": 0})
        .sort("created_at", -1)
        .skip(skip)
        .limit(page_size)
        .to_list(page_size)
...
@api_router.post("/lp")
async def create_lp_account(
    request: Request,
    lp_data: LPAccountCreate,
    user: dict = Depends(require_permission(Modules.LP_MANAGEMENT, Actions.CREATE)),
):
...
async def update_lp_account(
    request: Request,
    lp_id: str,
    lp_data: LPAccountUpdate,
    user: dict = Depends(require_permission(Modules.LP_MANAGEMENT, Actions.EDIT)),
):
...
async def create_lp_deposit(
    request: Request,
    lp_id: str,
    tx_data: LPTransactionCreate,
    user: dict = Depends(require_permission(Modules.LP_MANAGEMENT, Actions.CREATE)),
):
    ...
    if tx_data.treasury_account_id:
        treasury = await db.treasury_accounts.find_one(
            {"account_id": tx_data.treasury_account_id}, {"_id": 0}
        )
        if not treasury:
            raise HTTPException(status_code=404, detail="Treasury account not found")
        ...
        if treasury_currency.upper() != tx_data.currency.upper():
            treasury_deduct_amount = convert_currency(
                tx_data.amount, tx_data.currency, treasury_currency
            )

        if treasury.get("balance", 0) < treasury_deduct_amount:
            raise HTTPException(
                status_code=400,
                detail=f"Insufficient treasury balance. Required: {treasury_deduct_amount:,.2f} {treasury_currency}",
            )
        ...
        conversion_note = (
            f" (Converted from {tx_data.amount:,.2f} {tx_data.currency})"
            if treasury_currency.upper() != tx_data.currency.upper()
            else ""
        )
...
async def create_lp_withdrawal(
    request: Request,
    lp_id: str,
    tx_data: LPTransactionCreate,
    user: dict = Depends(require_permission(Modules.LP_MANAGEMENT, Actions.CREATE)),
    ...
    if tx_data.treasury_account_id:
        ...
        treasury_currency = treasury.get("currency", "USD")
        if treasury_currency.upper() != tx_data.currency.upper():
            treasury_add_amount = convert_currency(
                tx_data.amount, tx_data.currency, treasury_currency
            )
        ...
        await db.treasury_accounts.update_one(
            {"account_id": tx_data.treasury_account_id},
            {
                "$inc": {"balance": treasury_add_amount},
                "$set": {"updated_at": now.isoformat()},
            },
        )
    ...
    await db.lp_accounts.update_one(
        {"lp_id": lp_id},
        {
            "$inc": {
                "balance": -tx_data.amount,
                "total_withdrawals": tx_data.amount,
                "transaction_count": 1,
            },
            "$set": {"updated_at": now.isoformat()},
        },
    ...
    tx_doc.pop("_id", None)
    await log_activity(
        request, user, "create", "lp_management", "LP withdrawal recorded"
    )
...
async def export_lp_csv(
    user: dict = Depends(require_permission(Modules.LP_MANAGEMENT, Actions.EXPORT))
):
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={
            "Content-Disposition": f"attachment; filename=lp_export_{datetime.now().strftime('%Y%m%d')}.csv"
        },
    )
...
async def get_dealing_pnl_records(
    ...
    records = (
        await db.dealing_pnl.find(query, {"_id": 0})
        .sort("date", -1)
        .limit(limit)
        .to_list(limit)
    )
    ...
    for i, record in enumerate(records):
        ...
        prev_record = await db.dealing_pnl.find_one(
            {"date": {"$lt": record["date"]}}, {"_id": 0}, sort=[("date", -1)]
        )
        if prev_record:
...
async def create_dealing_pnl(
    request: Request,
    data: DealingPnLCreate,
    user: dict = Depends(require_permission(Modules.LP_MANAGEMENT, Actions.CREATE)),
):
    ...
    for entry in data.lp_entries or []:
        ...
        if not lp_entry["lp_name"]:
            lp = await db.lp_accounts.find_one(
                {"lp_id": entry.lp_id}, {"_id": 0, "lp_name": 1}
            )
            if lp:
    ...
    if existing:
        await db.dealing_pnl.update_one({"date": data.date}, {"$set": record})
        ...
        await log_activity(
            request, user, "create", "lp_management", "Created Dealing P&L entry"
        )

        return {
            "message": "Dealing P&L record created",
            "date": data.date,
...
async def get_dealing_pnl_by_date(
    date: str,
    user: dict = Depends(require_permission(Modules.LP_MANAGEMENT, Actions.VIEW)),
):
...
async def delete_dealing_pnl(
    request: Request,
    date: str,
    user: dict = Depends(require_permission(Modules.LP_MANAGEMENT, Actions.DELETE)),
):
    ...
    await log_activity(
        request, user, "delete", "lp_management", "Deleted Dealing P&L entry"
    )
...
async def send_dealing_pnl_email(
    date: str,
    user: dict = Depends(require_permission(Modules.LP_MANAGEMENT, Actions.EXPORT)),
):
    ...
    prev_record = await db.dealing_pnl.find_one(
        {"date": {"$lt": date}}, {"_id": 0}, sort=[("date", -1)]
    )
    ...
    try:
        ...
        with smtplib.SMTP(
            settings.get("smtp_host", "smtp.gmail.com"), settings.get("smtp_port", 587)
        ) as server:
            server.starttls()
            server.login(
...
async def get_psps(user: dict = Depends(require_listing_or_view(Modules.PSP))):
    ...
    treasury_ids = list(
        set(
            psp.get("settlement_destination_id")
            for psp in psps
    ...
    treasury_map = {}
    if treasury_ids:
        treasuries = await db.treasury_accounts.find(
            {"account_id": {"$in": treasury_ids}}, {"_id": 0}
    ...
    for psp in psps:
        if psp.get("settlement_destination_id"):
            ...
            psp["settlement_destination_name"] = (
...
async def create_psp(
    request: Request,
    psp_data: PSPCreate,
    user: dict = Depends(require_permission(Modules.PSP, Actions.CREATE)),
):

    # Verify settlement destination exists
    dest = await db.treasury_accounts.find_one(
        {"account_id": psp_data.settlement_destination_id}, {"_id": 0}
    )
    if not dest:
        raise HTTPException(
...
async def update_psp(
    ...
    if updates.get("settlement_destination_id"):
        dest = await db.treasury_accounts.find_one(
            {"account_id": updates["settlement_destination_id"]}, {"_id": 0}
        )
        if not dest:
            raise HTTPException(
                status_code=404, detail="Settlement destination account not found"
...
async def delete_psp(
    request: Request,
    psp_id: str,
    user: dict = Depends(require_permission(Modules.PSP, Actions.DELETE)),
):
...
async def get_all_settlements(
    ...
    if status:
        query["status"] = status
    settlements = (
        await db.psp_settlements.find(query, {"_id": 0})
        .sort("created_at", -1)
        .to_list(limit)
    )
    return settlements
...
async def create_settlement(
    ...
    final_reserve_fund = (
        total_tx_reserve if total_tx_reserve > 0 else reserve_fund_amount
    )
    ...
    settlement_doc = {
        ...
        "reserve_fund_rate": psp.get(
            "reserve_fund_rate", psp.get("chargeback_rate", 0)
        ),
        ...
        "total_deductions": commission_amount
        + final_reserve_fund
        + total_extra_charges
        + total_gateway_fees,
    ...
    return await db.psp_settlements.find_one(
...
async def complete_settlement(
    ...
    dest = await db.treasury_accounts.find_one(
        {"account_id": settlement["settlement_destination_id"]}, {"_id": 0}
    )
...
async def get_psp_all_transactions(
    ...
    if status and status != "all":
        ...
        # Reconciliation view: only settled, non-rejected money movements —
        # drops rejected/cancelled and anything still pending settlement.
...
async def update_psp_transaction_charges(
    ...
    if tx.get("destination_type") != "psp":
        raise HTTPException(
            status_code=400, detail="Transaction is not a PSP transaction"
        )

    if tx.get("settled"):
        raise HTTPException(
            status_code=400, detail="Cannot update charges on settled transaction"
        )
...
async def record_psp_payment(
    ...
    psp = await db.psps.find_one({"psp_id": tx.get("psp_id")}, {"_id": 0})
    dest_account_id = destination_account_id or (
        psp.get("settlement_destination_id") if psp else None
    )
    ...
    variance = (
        settle_amount - expected_amount if actual_amount_received is not None else 0
    )
    ...
    conversion_note = (
        f" (Converted: {tx_currency} {settle_amount:,.2f} -> {dest_currency} {treasury_amount:,.2f})"
        if tx_currency != dest_currency
        else ""
    )
    treasury_tx = {
    ...
    settlement_doc = {
        ...
        "commission_amount": commission_amount,
        "reserve_fund_rate": (
            psp.get("reserve_fund_rate", psp.get("chargeback_rate", 0)) if psp else 0
        ),
...
async def settle_psp_transaction(
    ...
    if tx.get("status") not in [
        TransactionStatus.APPROVED,
        TransactionStatus.COMPLETED,
        TransactionStatus.PENDING,
    ]:
        raise HTTPException(
            status_code=400, detail="Transaction cannot be settled in current status"
        )
...
async def backfill_psp_settlements(
    user: dict = Depends(require_permission(Modules.PSP, Actions.CREATE))
):
    ...
    settled_txs = await db.transactions.find(
        {
            "destination_type": "psp",
            "settled": True,
            "$or": [{"settlement_id": {"$exists": False}}, {"settlement_id": None}],
        },
        {"_id": 0},
    ).to_list(10000)
    ...
    for tx in settled_txs:
        ...
        settlement_doc = {
            ...
            "psp_id": tx.get("psp_id"),
            "psp_name": tx.get(
                "psp_name", psp.get("psp_name") if psp else "Unknown PSP"
            ),
            "gross_amount": gross_amount,
            "commission_rate": psp.get("commission_rate", 0) if psp else 0,
            "commission_amount": commission_amount,
            "reserve_fund_rate": (
                psp.get("reserve_fund_rate", psp.get("chargeback_rate", 0))
                if psp
                else 0
            ),
            "reserve_fund_amount": reserve_fund_amount,
            ...
            "settlement_destination_id": tx.get(
                "settlement_destination_id",
                psp.get("settlement_destination_id") if psp else None,
            ),
            "status": PSPSettlementStatus.COMPLETED,
    ...
    return {
        "message": f"Successfully backfilled {backfilled} settlements",
        "count": backfilled,
    }
...
async def sync_treasury_ie_dates(
    user: dict = Depends(require_permission(Modules.TREASURY, Actions.EDIT))
):
    ...
    txs = await db.treasury_transactions.find(
        {"income_expense_id": {"$exists": True}}, {"_id": 0}
    ).to_list(10000)
    ...
    return {
        "message": f"Successfully fixed {fixed} treasury transaction dates",
        "count": fixed,
    }
...
async def assign_role_ids(
    ...
    users = await db.users.find({"role_id": {"$exists": False}}, {"_id": 0}).to_list(
        1000
    )
    ...
    for u in unique_users:
        ...
        await db.users.update_one(
            {"user_id": u["user_id"]}, {"$set": {"role_id": new_role_id}}
        )
        updated += 1
...
async def get_psp_reserve_funds(
    psp_id: str, user: dict = Depends(require_permission(Modules.PSP, Actions.VIEW))
):
    ...
    for tx in txs:
        ...
        release_date = (
            created_dt + timedelta(days=holding_days)
            if holding_days > 0
            else created_dt
        )
...
async def release_reserve_fund(
    request: Request,
    transaction_id: str,
    user: dict = Depends(require_permission(Modules.PSP, Actions.APPROVE)),
):
    ...
    if psp and psp.get("settlement_destination_id") and rf_amount > 0:
        dest = await db.treasury_accounts.find_one(
            {"account_id": psp["settlement_destination_id"]}, {"_id": 0}
        )
        tx_currency = tx.get("currency", "USD")
        dest_currency = dest.get("currency", "USD") if dest else "USD"
        ...
        await db.treasury_accounts.update_one(
            ...
            {
                "$inc": {"balance": treasury_amount},
                "$set": {"updated_at": now.isoformat()},
            },
        )
        conversion_note = (
            f" (Converted: {tx_currency} {rf_amount:,.2f} -> {dest_currency} {treasury_amount:,.2f})"
            if tx_currency != dest_currency
            else ""
        )
...
async def bulk_release_reserve_funds(
    request: Request,
    user: dict = Depends(require_permission(Modules.PSP, Actions.APPROVE)),
):
    ...
    for tx_id in tx_ids:
        tx = await db.transactions.find_one(
            {"transaction_id": tx_id, "reserve_fund_released": {"$ne": True}},
            {"_id": 0},
        )
        if not tx:
            continue
        ...
        if not rf_amount or rf_amount <= 0:
            psp_for_rate = await db.psps.find_one(
                {"psp_id": tx.get("psp_id")}, {"_id": 0}
            )
            if psp_for_rate:
                rf_rate = (
                    psp_for_rate.get(
                        "reserve_fund_rate", psp_for_rate.get("chargeback_rate", 0)
                    )
                    / 100
                )
                rf_amount = round(tx.get("amount", 0) * rf_rate, 2)
    ...
    return {
        "message": f"Released {released_count} reserve funds",
        "total_released": round(total_released, 2),
...
async def get_global_reserve_fund_summary(
    user: dict = Depends(require_permission(Modules.PSP, Actions.VIEW))
):
    """Get global reserve fund summary across all PSPs."""
    psps = await db.psps.find(
        {"status": PSPStatus.ACTIVE},
        {
            "_id": 0,
            "psp_id": 1,
            ...
            if tx.get("reserve_fund_released"):
                ...
                try:
                    created = datetime.fromisoformat(
                        tx.get("created_at", "").replace("Z", "+00:00")
                    )
...
async def get_vendors(
    ...
    cache_key = get_cache_key(
        "vendors:list", page=page, page_size=page_size, search=search
    )
    ...
    vendors = (
        await db.vendors.find(query, {"_id": 0})
        .sort("created_at", -1)
        .skip(skip)
        .limit(page_size)
        .to_list(page_size)
    ...
    treasury_ids = list(
        set(
            v.get("settlement_destination_id")
            for v in vendors
            if v.get("settlement_destination_id")
        )
    )
    treasury_map = {}
    if treasury_ids:
    ...
    for ltx in loan_txs_all:
        if ltx.get("source_vendor_id"):
            loan_tx_by_vendor[ltx["source_vendor_id"]].append(
                {"type": "out", "tx": ltx}
            )
        if ltx.get("credit_to_vendor_id"):
            loan_tx_by_vendor[ltx["credit_to_vendor_id"]].append(
                {"type": "in", "tx": ltx}
            )
    ...
    for vendor in vendors:
        if vendor.get("settlement_destination_id"):
            ...
            vendor["settlement_destination_name"] = (
                dest["account_name"] if dest else "Unknown"
            )
            vendor["settlement_destination_bank"] = (
                dest.get("bank_name") if dest else None
            )
...
async def create_vendor(
    ...
    await log_activity(
        ...
        "create",
        "exchangers",
        f"Created exchanger: {vendor_data.vendor_name}",
...
async def delete_vendor(
    request: Request,
    vendor_id: str,
    user: dict = Depends(require_permission(Modules.EXCHANGERS, Actions.DELETE)),
):
...
async def get_vendor_transactions(
    ...
    if user.get("role") == UserRole.VENDOR:
        user_vendor = await db.vendors.find_one(
...
async def get_my_vendor_info(user: dict = Depends(require_vendor)):
    ...
    for curr, loan_item in loan_tx_map.items():
        ...
        currency_data[curr]["commission_usd"] += loan_item.get(
            "loan_commission_amount", 0
        )
        currency_data[curr]["commission_base"] += loan_item.get(
            "loan_commission_base", 0
        )
...
async def get_vendor_my_settlements(
    ...
    return await paginate_query(
        db.vendor_settlements, {"vendor_id": vendor["vendor_id"]}, page, page_size
    )
...
async def vendor_export_excel(
    ...
    for col, h in enumerate(headers, 1):
        ...
        cell.fill = openpyxl.styles.PatternFill(
            start_color="0B3D91", end_color="0B3D91", fill_type="solid"
        )
    ...
    for i, tx in enumerate(txs, 2):
        ...
        ws.cell(
            row=i, column=5, value=tx.get("base_currency") or tx.get("currency", "USD")
        )
        ...
        ws.cell(
            row=i,
            column=9,
            value=tx.get("created_at", "")[:10] if tx.get("created_at") else "",
        )
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=transactions_{vendor['vendor_name'].replace(' ', '_')}.xlsx"
        },
    )
...
async def vendor_export_pdf(
    ...
    elements.append(
        Paragraph(f"Transaction Report — {vendor['vendor_name']}", title_style)
    )
    filter_parts = []
    if status and status != "all":
    ...
    elements.append(
        Paragraph(
            f"Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}  |  {' | '.join(filter_parts) if filter_parts else 'All transactions'}",
            subtitle_style,
        )
    )
    ...
    elements.append(
        Paragraph(
            f"Total: {len(txs)} transactions  |  Deposits: {total_deposits}  |  Withdrawals: {total_withdrawals}  |  Total Commission: ${total_commission:,.2f}",
            subtitle_style,
        )
    )
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=transactions_{vendor['vendor_name'].replace(' ', '_')}.pdf"
        },
    )
...
async def vendor_approve_transaction(
    ...
    if tx["transaction_type"] == TransactionType.WITHDRAWAL and not tx.get(
        "vendor_proof_image"
    ):
        raise HTTPException(
            status_code=400,
            detail="Please upload proof screenshot before approving withdrawal",
        )
    ...
    if tx["transaction_type"] == TransactionType.DEPOSIT:
        if tx_mode == "cash":
            commission_rate = (
                vendor.get(
                    "deposit_commission_cash", vendor.get("deposit_commission", 0)
                )
                / 100
            )
        else:
        ...
        if tx_mode == "cash":
            commission_rate = (
                vendor.get(
                    "withdrawal_commission_cash", vendor.get("withdrawal_commission", 0)
                )
                / 100
            )
        else:
    ...
    await log_activity(
        request, user, "approve", "transactions", "Vendor approved transaction"
    )
...
async def vendor_reject_transaction(
    ...
    await log_activity(
        request, user, "reject", "transactions", "Vendor rejected transaction"
    )
...
async def get_vendor_loan_transactions(
    request: Request,
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=200),
    fetch_all: bool = False,
    ...
    loan_ids = list(
        set([tx.get("loan_id") for tx in transactions if tx.get("loan_id")])
    )
    loan_map = {}
    if loan_ids:
        loans = await db.loans.find({"loan_id": {"$in": loan_ids}}, {"_id": 0}).to_list(
            500
        )
...
async def vendor_reject_loan_transaction(
    ...
    await log_activity(
        request, user, "reject", "loans", "Vendor rejected loan transaction"
    )
...
async def get_vendor_settlements(
    vendor_id: str,
    user: dict = Depends(require_permission(Modules.EXCHANGERS, Actions.VIEW)),
):
    settlements = (
        await db.vendor_settlements.find({"vendor_id": vendor_id}, {"_id": 0})
        .sort("created_at", -1)
        .to_list(1000)
    )
    return settlements
...
async def get_settlement_statement(
    ...
    settlement = await db.vendor_settlements.find_one(
        {"settlement_id": settlement_id}, {"_id": 0}
    )
    if not settlement:
        raise HTTPException(status_code=404, detail="Settlement not found")
    ...
    vendor = await db.vendors.find_one(
        ...
        {"_id": 0, "vendor_name": 1, "contact_person": 1, "email": 1, "phone": 1},
    )
    return {
...
async def get_pending_settlements(
    ...
    settlements = (
        await db.vendor_settlements.find(
            {"status": VendorSettlementStatus.PENDING}, {"_id": 0}
        )
        .sort("created_at", -1)
        .to_list(1000)
    )
...
async def approve_settlement(
    ...
    user: dict = Depends(require_permission(Modules.EXCHANGERS, Actions.APPROVE)),
):
...
async def get_transactions(
    ...
    if client_ids:
        clients = await db.clients.find(
            {"client_id": {"$in": client_ids}}, {"_id": 0, "client_id": 1, "email": 1}
...
async def approve_income_expense(request: Request, entry_id: str, approval_date: Optional[str] = None, user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.APPROVE))):
    ...
    if treasury:
        ...
        # Determine the correct amount in the treasury's currency
...
async def create_transaction(
    ...
    for img in (proof_images or []):
        if img and img.filename:
            content = await img.read()
            url = upload_to_r2(content, img.filename, img.content_type or "application/octet-stream", "proofs")
...
async def update_transaction(
    ...
    if updates.get("status") in [
        ...
        if updates.get("status") == TransactionStatus.COMPLETED and tx.get(
            "destination_account_id"
        ):
            balance_change = (
                tx["amount"]
...
async def assign_transaction_destination(
    ...
    if tx["status"] != TransactionStatus.PENDING:
        raise HTTPException(
            status_code=400, detail="Only pending transactions can be edited"
        )
    ...
    if dest_type == "psp" and psp_id:
        ...
        comm_rate = (
            vendor.get("withdrawal_commission", 0)
            if tx["transaction_type"] == "withdrawal"
            else vendor.get("deposit_commission", 0)
        )
        if comm_rate > 0:
            updates["vendor_commission_rate"] = comm_rate
            updates["vendor_commission_base_amount"] = round(
                base_amount * comm_rate / 100, 2
            )
    ...
    await log_activity(
        ...
        "edit",
        "transactions",
        f"Assigned transaction to {updates.get('vendor_name', dest_type)}",
    )
...
async def get_transaction_requests(
    ...
    page_size: int = Query(20, ge=1, le=200),
    user: dict = Depends(
        require_permission(Modules.TRANSACTION_REQUESTS, Actions.VIEW)
    ),
):
    query = {}
...
async def get_transaction_request(
    request_id: str,
    user: dict = Depends(
        require_permission(Modules.TRANSACTION_REQUESTS, Actions.VIEW)
    ),
...
async def process_transaction_request(
    ...
    data: dict = Body(...),
    user: dict = Depends(
        require_permission(Modules.TRANSACTION_REQUESTS, Actions.APPROVE)
    ),
):
    ...
    if req["status"] != "pending":
        raise HTTPException(
            status_code=400, detail="Only pending requests can be processed"
        )
...
async def delete_transaction_request(
    request: Request,
    request_id: str,
    user: dict = Depends(
        require_permission(Modules.TRANSACTION_REQUESTS, Actions.DELETE)
    ),
):
    req = await db.transaction_requests.find_one({"request_id": request_id}, {"_id": 0})
    if not req:
    ...
    await log_activity(
        request, user, "delete", "transaction_requests", "Deleted request"
    )
...
async def get_dashboard_stats(
    user: dict = Depends(require_permission(Modules.DASHBOARD, Actions.VIEW))
):
    # Get client stats
    total_clients = await db.clients.count_documents({})
    approved_clients = await db.clients.count_documents(
        {"kyc_status": ClientStatus.APPROVED}
    )
    pending_clients = await db.clients.count_documents(
        {"kyc_status": ClientStatus.PENDING}
    )

    # Get treasury stats
    total_treasury = await db.treasury_accounts.count_documents({})
    active_treasury = await db.treasury_accounts.count_documents(
        {"status": TreasuryAccountStatus.ACTIVE}
    )

    # Get total treasury balance using manual FX rates
    all_accounts = await db.treasury_accounts.find(
        {"status": TreasuryAccountStatus.ACTIVE}, {"_id": 0}
    ).to_list(1000)
    fx_settings = await db.app_settings.find_one(
        {"setting_type": "manual_fx_rates"}, {"_id": 0}
    )
...
async def get_client_analytics(
    ...
    kyc_pipeline = [{"$group": {"_id": "$kyc_status", "count": {"$sum": 1}}}]
    ...
    return {
        "kyc_distribution": [
            {"status": s["_id"], "count": s["count"]} for s in kyc_stats
        ],
        "country_distribution": [
            {"country": c["_id"], "count": c["count"]} for c in country_stats
        ],
    }
...
async def get_recent_activity(
    ...
    transactions = (
        await db.transactions.find({}, {"_id": 0}).sort("created_at", -1).to_list(limit)
    )
    clients = (
        await db.clients.find({}, {"_id": 0}).sort("created_at", -1).to_list(limit)
    )

    return {"recent_transactions": transactions, "recent_clients": clients}
...
async def seed_demo_data():
    ...
    accountant = await db.users.find_one(
        {"email": "accountant@fxbroker.com"}, {"_id": 0}
    )
    if not accountant:
        accountant_doc = {
            "user_id": f"user_{uuid.uuid4().hex[:12]}",
    ...
    treasury_accounts_data = [
        ...
        {
            "account_name": "Client Funds Account",
            "account_type": "bank",
    ...
    for ta in treasury_accounts_data:
        existing = await db.treasury_accounts.find_one(
            {"account_name": ta["account_name"]}, {"_id": 0}
        )
        if not existing:
            account_id = f"treasury_{uuid.uuid4().hex[:12]}"
            ta_doc = {
    ...
    vendor_data = [
        {
            ...
            "description": "Premium exchange vendor",
        },
        {
            "vendor_name": "FastPay Solutions",
            "email": "vendor2@fxbroker.com",
            "password": "vendor123",
            "deposit_commission": 2.0,
            "withdrawal_commission": 2.5,
            "bank_settlement_commission": 0.3,
            "cash_settlement_commission": 0.8,
    ...
    for v in vendor_data:
        existing = await db.vendors.find_one(
            {"vendor_name": v["vendor_name"]}, {"_id": 0}
        )
        if not existing and treasury_ids:
            vendor_id = f"vendor_{uuid.uuid4().hex[:12]}"
            user_id = f"user_{uuid.uuid4().hex[:12]}"
...
async def get_income_expenses(
    ...
    treasury_ids = list(
        set(
            e.get("treasury_account_id")
            for e in entries
            if e.get("treasury_account_id")
        )
    )
    treasury_map = {}
    if treasury_ids:
    ...
    for entry in entries:
        if entry.get("treasury_account_id"):
            entry["treasury_account_name"] = treasury_map.get(
                entry["treasury_account_id"], "Unknown"
            )
...
async def get_income_expense_summary(
    ...
    entries = await db.income_expenses.find(
        {**query, "converted_to_loan": {"$ne": True}}, {"_id": 0}
    ).to_list(10000)

    # Use .get() with fallback to handle entries without amount_usd
    total_income = sum(
        e.get(
            "amount_usd", convert_to_usd(e.get("amount", 0), e.get("currency", "USD"))
        )
        for e in entries
        if e["entry_type"] == IncomeExpenseType.INCOME
    )
    total_expense = sum(
        e.get(
            "amount_usd", convert_to_usd(e.get("amount", 0), e.get("currency", "USD"))
        ...
        for e in entries
        if e["entry_type"] == IncomeExpenseType.EXPENSE
    )
    ...
    for entry in entries:
        ...
        amount = entry.get(
            "amount_usd",
            convert_to_usd(entry.get("amount", 0), entry.get("currency", "USD")),
        )
...
async def get_monthly_report(
    ...
    for entry in entries:
        ...
        if month_str in monthly_data:
            amt_usd = entry.get("amount_usd") or convert_to_usd(
                entry.get("amount", 0), entry.get("currency", "USD")
            )
            if entry["entry_type"] == IncomeExpenseType.INCOME:
...
async def get_categories(
    ...
    custom_income = await db.income_expenses.distinct(
        "custom_category", {"entry_type": "income", "custom_category": {"$ne": None}}
    )
    custom_expense = await db.income_expenses.distinct(
        "custom_category", {"entry_type": "expense", "custom_category": {"$ne": None}}
    )
...
async def get_ie_import_template_route(
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.EXPORT))
):
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": "attachment; filename=income_expenses_template.xlsx"
        },
    )
...
async def bulk_import_ie_entries_route(
    ...
    treasury = await db.treasury_accounts.find_one(
        {"account_id": treasury_account_id}, {"_id": 0}
    )
    if not treasury:
        raise HTTPException(status_code=404, detail="Treasury account not found")
    ...
    if "amount" not in col_map:
        raise HTTPException(
            status_code=400, detail="Excel file must have an 'amount' column"
        )
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        try:
            ...
            entry_type_val = (
                row[col_map.get("entry_type", col_map.get("type", 0))]
                if "entry_type" in col_map or "type" in col_map
                else "expense"
            )
            entry_type = (
                "income"
                if str(entry_type_val).lower().strip()
                in ["income", "in", "credit", "cr"]
                else "expense"
            )
            category = (
                str(row[col_map.get("category", 0)] or "other")
                .lower()
                .replace(" ", "_")
            )
    ...
    await log_activity(
        request, user, "create", "income_expenses", "Bulk imported I&E entries"
    )
...
async def create_income_expense(
    entry_data: IncomeExpenseCreate,
    request: Request,
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.CREATE)),
):
    ...
    if not entry_data.category and not entry_data.ie_category_id:
        raise HTTPException(
            status_code=400, detail="Either a category or custom category is required"
        )

    if not entry_data.vendor_id and not entry_data.treasury_account_id:
        raise HTTPException(
            status_code=400,
            detail="Either an exchanger or treasury account is required",
        )
    ...
    if entry_data.vendor_id:
        vendor_info = await db.vendors.find_one(
            {"vendor_id": entry_data.vendor_id}, {"_id": 0}
        )
        if not vendor_info:
            raise HTTPException(status_code=404, detail="Exchanger not found")
    ...
    if entry_data.vendor_supplier_id:
        vendor_supplier_info = await db.vendor_suppliers.find_one(
            {"supplier_id": entry_data.vendor_supplier_id}, {"_id": 0}
        )
    ...
    if entry_data.client_id:
        client_info = await db.clients.find_one(
            {"client_id": entry_data.client_id}, {"_id": 0}
        )
        if not client_info:
            raise HTTPException(status_code=404, detail="Client not found")
    ...
    if entry_data.ie_category_id:
        ie_category_info = await db.ie_categories.find_one(
            {"category_id": entry_data.ie_category_id}, {"_id": 0}
        )
    ...
    if entry_data.treasury_account_id and not entry_data.vendor_id:
        treasury = await db.treasury_accounts.find_one(
            {"account_id": entry_data.treasury_account_id}, {"_id": 0}
        )
        if not treasury:
            raise HTTPException(status_code=404, detail="Treasury account not found")
    ...
    if vendor_info:
        ...
        if entry_data.entry_type == "income":
            if tx_mode == "cash":
                ie_commission_rate = vendor_info.get(
                    "deposit_commission_cash", vendor_info.get("deposit_commission", 0)
                )
            else:
        ...
        if ie_commission_rate > 0:
            ...
            actual_tx_amount = (
                entry_data.base_amount if entry_data.base_amount else entry_data.amount
            )
            actual_tx_currency = (
                entry_data.base_currency
                if entry_data.base_currency
                else entry_data.currency
            )
            ie_commission_base_amount = round(
                actual_tx_amount * ie_commission_rate / 100, 2
            )
    ...
    entry_doc = {
        ...
        "vendor_supplier_name": (
            vendor_supplier_info["name"] if vendor_supplier_info else None
        ),
        ...
        "collecting_person_name": (
            entry_data.collecting_person_name
            if entry_data.transaction_mode == "cash"
            else None
        ),
        "collecting_person_number": (
            entry_data.collecting_person_number
            if entry_data.transaction_mode == "cash"
            else None
        ),
        "vendor_commission_rate": (
            ie_commission_rate if ie_commission_rate > 0 else None
        ),
        "vendor_commission_amount": (
            ie_commission_amount if ie_commission_amount > 0 else None
        ),
        "vendor_commission_base_amount": (
            ie_commission_base_amount if ie_commission_base_amount > 0 else None
        ),
        "vendor_commission_base_currency": (
            ie_commission_base_currency if ie_commission_base_amount > 0 else None
        ),
        "status": status,
        "converted_to_loan": False,
        "loan_id": None,
    ...
    if vendor_info and entry_data.vendor_id:
        ...
        amt_display = (
            ...
            else f"{entry_data.amount:,.2f} {entry_data.currency}"
        )
        asyncio.create_task(
            send_exchanger_notification(
                "ie",
                entry_data.vendor_id,
                {
                    "entry_type": entry_data.entry_type,
...
async def update_income_expense(
    request: Request,
    entry_id: str,
    update_data: IncomeExpenseUpdate,
...
async def delete_income_expense(
    entry_id: str,
    request: Request,
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.DELETE)),
):
    ...
    await log_activity(
        ...
        "delete",
        "income_expenses",
        f"Deleted {entry['entry_type']}: {entry.get('description', 'N/A')}",
...
class ConvertToLoanRequest(BaseModel):
    ...
    due_date: str
    treasury_account_id: Optional[str] = (
        None  # Optional - will use expense's treasury if not provided
    )
    notes: Optional[str] = None


@api_router.get("/loans/borrowers")
async def get_loan_borrowers(
    ...
    vendors = await db.vendors.find(
        {"status": "active"}, {"_id": 0, "vendor_id": 1, "vendor_name": 1, "email": 1}
    ).to_list(length=1000)
    ...
    for b in loan_borrowers:
        if b and b not in seen_names:
            borrowers.append(
                {"name": b, "vendor_id": None, "email": None, "type": "historical"}
            )
            seen_names.add(b)
...
async def convert_expense_to_loan(
    request: Request,
    entry_id: str,
    req: ConvertToLoanRequest,
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.EDIT)),
):
    ...
    if entry["entry_type"] != "expense":
        raise HTTPException(
            status_code=400, detail="Only expenses can be converted to loans"
        )
    if entry.get("converted_to_loan"):
        raise HTTPException(
            status_code=400, detail="This expense has already been converted to a loan"
        )
    ...
    await log_activity(
        request, user, "edit", "income_expenses", "Converted expense to loan"
    )
...
async def vendor_approve_ie(
    request: Request,
    entry_id: str,
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.APPROVE)),
):
    ...
    if entry.get("status") != "pending_vendor":
        raise HTTPException(
            status_code=400, detail="Entry is not pending vendor approval"
        )
    ...
    if not entry.get("vendor_proof_image"):
        raise HTTPException(
            status_code=400, detail="Please upload proof screenshot before approving"
        )
    ...
    amount_usd = entry.get("amount_usd") or convert_to_usd(
        amount, entry.get("currency", "USD")
    )
    ...
    await log_activity(
        request, user, "approve", "income_expenses", "Vendor approved I&E entry"
    )
    ...
    await db.income_expenses.update_one(
        {"entry_id": entry_id},
        {
            "$set": {
                "status": "rejected",
                "rejection_reason": reason,
                "vendor_rejected_at": now.isoformat(),
            }
        },
    )

    await log_activity(
        request, user, "reject", "income_expenses", "Vendor rejected I&E entry"
    )
...
async def get_vendor_suppliers(
    ...
    suppliers = (
        await db.vendor_suppliers.find(query, {"_id": 0}).sort("name", 1).to_list(1000)
    )
    return suppliers


@api_router.get("/vendor-suppliers/{supplier_id}")
async def get_vendor_supplier(
    supplier_id: str,
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.VIEW)),
):
    """Get a specific vendor supplier"""
    supplier = await db.vendor_suppliers.find_one(
...
async def create_vendor_supplier(
    request: Request,
    data: VendorSupplierCreate,
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.CREATE)),
):
    """Create a new vendor supplier (for services like rent, utilities)"""
    # Check for duplicate name
    existing = await db.vendor_suppliers.find_one(
        {"name": {"$regex": f"^{data.name}$", "$options": "i"}}
    )
    if existing:
        raise HTTPException(
            status_code=400, detail="A supplier with this name already exists"
        )
    ...
    await log_activity(
        request, user, "create", "income_expenses", "Created vendor supplier"
    )
...
async def update_vendor_supplier(
    request: Request,
    supplier_id: str,
    data: VendorSupplierUpdate,
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.EDIT)),
):
    ...
    if update_dict:
        ...
        await db.vendor_suppliers.update_one(
            {"supplier_id": supplier_id}, {"$set": update_dict}
        )

    updated = await db.vendor_suppliers.find_one(
        {"supplier_id": supplier_id}, {"_id": 0}
    )
    await log_activity(
        request, user, "edit", "income_expenses", "Updated vendor supplier"
    )
...
async def delete_vendor_supplier(
    request: Request,
    supplier_id: str,
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.DELETE)),
):
    ...
    linked_entries = await db.income_expenses.count_documents(
    ...
    if linked_entries > 0:
        ...
        await db.vendor_suppliers.update_one(
            ...
            {
                "$set": {
                    "status": VendorSupplierStatus.INACTIVE,
                    "updated_at": datetime.now(timezone.utc).isoformat(),
                }
            },
        )
    ...
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Vendor supplier not found")
    await log_activity(
        request, user, "delete", "income_expenses", "Deleted vendor supplier"
    )
...
async def get_ie_categories(
    ...
    categories = (
        await db.ie_categories.find(query, {"_id": 0}).sort("name", 1).to_list(500)
    )
    return categories


@api_router.get("/ie-categories/{category_id}")
async def get_ie_category(
    category_id: str,
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.VIEW)),
):
...
async def create_ie_category(
    request: Request,
    data: IECategoryCreate,
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.CREATE)),
):
    ...
    await log_activity(
        request, user, "create", "income_expenses", "Created I&E category"
    )
...
async def update_ie_category(
    request: Request,
    category_id: str,
    data: IECategoryUpdate,
    user: dict = Depends(require_permission(Modules.INCOME_EXPENSES, Actions.EDIT)),
):
    """Update a category"""
...
async def delete_ie_category(
    ...
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Category not found")
    await log_activity(
        request, user, "delete", "income_expenses", "Deleted I&E category"
    )
...
async def get_loan_dashboard(
    ...
    for loan in loans:
        ...
        interest_usd = convert_to_usd(
            loan.get("total_interest", 0), loan.get("currency", "USD")
        )
        repaid_usd = convert_to_usd(
            loan.get("total_repaid", 0), loan.get("currency", "USD")
        )
        ...
        if loan.get("total_repaid", 0) > loan["amount"]:
            total_interest_earned += convert_to_usd(
                loan["total_repaid"] - loan["amount"], loan.get("currency", "USD")
            )
        ...
        currency_exposure[currency]["outstanding"] += max(
            0,
            loan["amount"]
    ...
    top_borrowers = sorted(
        borrower_data.items(), key=lambda x: x[1]["outstanding"], reverse=True
    )[:5]

    # Collection rate (this month)
    start_of_month = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    this_month_repayments = await db.loan_repayments.find(
        {"payment_date": {"$gte": start_of_month.isoformat()}}, {"_id": 0}
    ).to_list(10000)

    this_month_collected = sum(
        convert_to_usd(r.get("amount", 0), r.get("currency", "USD"))
        for r in this_month_repayments
    )
...
async def get_loan_transactions(
    ...
    treasury_ids = list(
        set(
            [
                tx.get("treasury_account_id")
                for tx in transactions
                if tx.get("treasury_account_id")
            ]
        )
    )
    vendor_ids = list(
        set(
            [
                tx.get("credit_to_vendor_id")
                for tx in transactions
                if tx.get("credit_to_vendor_id")
            ]
            + [
                tx.get("source_vendor_id")
                for tx in transactions
                if tx.get("source_vendor_id")
            ]
        )
    )
    ...
    if vendor_ids:
        ...
        vendor_map = {
            v["vendor_id"]: v.get("name") or v.get("vendor_name") for v in vendors
        }
    ...
    for tx in transactions:
        ...
        if tx.get("transaction_type") == "disbursement" and tx.get("loan_id"):
            ...
            if loan:
                if loan.get("source_treasury_id"):
                    source_treasury = await db.treasury_accounts.find_one(
                        {"account_id": loan["source_treasury_id"]}, {"_id": 0}
                    )
                    tx["treasury_account_name"] = (
                        source_treasury["account_name"] if source_treasury else None
                    )
                elif loan.get("source_vendor_id"):
                    source_vendor = await db.vendors.find_one(
                        {"vendor_id": loan["source_vendor_id"]}, {"_id": 0}
                    )
                    tx["source_vendor_name"] = (
                        source_vendor.get("name") or source_vendor.get("vendor_name")
                        if source_vendor
                        else None
                    )
...
async def get_vendor_borrowers(
    user: dict = Depends(require_permission(Modules.LOANS, Actions.VIEW))
):
...
async def get_loans(
    ...
    loans = (
        await db.loans.find(query, {"_id": 0})
        .sort("created_at", -1)
        .skip(skip)
        .limit(page_size)
        .to_list(page_size)
    ...
    for loan in loans:
        if loan.get("source_treasury_id"):
            loan["source_treasury_name"] = treasury_map.get(
                loan["source_treasury_id"], "Unknown"
            )

        # Calculate outstanding balance
        loan["outstanding_balance"] = (
            loan["amount"] + loan.get("total_interest", 0) - loan.get("total_repaid", 0)
        )
        ...
        # USD equivalent of total repaid
        loan["total_repaid_usd"] = convert_to_usd(
            loan.get("total_repaid", 0), loan.get("currency", "USD")
        )
...
async def get_loan(
    loan_id: str, user: dict = Depends(require_permission(Modules.LOANS, Actions.VIEW))
):
    ...
    repayments = (
        await db.loan_repayments.find({"loan_id": loan_id}, {"_id": 0})
        .sort("payment_date", -1)
        .to_list(1000)
    )
    for rep in repayments:
        if rep.get("treasury_account_id"):
            acc = await db.treasury_accounts.find_one(
                {"account_id": rep["treasury_account_id"]}, {"_id": 0}
            )
...
async def create_loan(
    ...
    if not loan_data.treasury_account_id and not loan_data.disburse_from_vendor_id:
        raise HTTPException(
            status_code=400,
            detail="Please select Treasury or Exchanger to disburse from",
        )
    ...
    if loan_data.disburse_from_vendor_id:
        disburse_vendor = await db.vendors.find_one(
            {"vendor_id": loan_data.disburse_from_vendor_id}, {"_id": 0}
        )
    ...
    if loan_data.vendor_id:
        vendor = await db.vendors.find_one(
            {"vendor_id": loan_data.vendor_id}, {"_id": 0}
        )
    ...
    total_interest = round(
        loan_data.amount * (loan_data.interest_rate / 100) * years, 2
    )
    ...
    loan_doc = {
        ...
        "vendor_name": (
            vendor.get("vendor_name") or vendor.get("name") if vendor else None
        ),
    ...
    if disburse_vendor:
        ...
        if vendor_commission_rate > 0:
            ...
            vendor_commission_amount = round(
                loan_data.amount * vendor_commission_rate / 100, 2
            )
    ...
    if treasury:
        ...
        loan_doc["source_vendor_name"] = disburse_vendor.get(
            "name"
        ) or disburse_vendor.get("vendor_name")
    ...
    await log_activity(
        ...
        "create",
        "loans",
        f"Created loan to {loan_data.borrower_name}: {loan_data.amount} {loan_data.currency}",
    ...
    if loan_data.disburse_from_vendor_id:
        ...
        asyncio.create_task(
            send_exchanger_notification(
                "loan",
                loan_data.disburse_from_vendor_id,
                {
...
async def record_loan_repayment(
    ...
    if not repayment.treasury_account_id and not repayment.credit_to_vendor_id:
        raise HTTPException(
            status_code=400, detail="Please select Treasury or Exchanger to credit"
        )
    ...
    if repayment.treasury_account_id:
        treasury = await db.treasury_accounts.find_one(
            {"account_id": repayment.treasury_account_id}, {"_id": 0}
        )
        if not treasury:
            raise HTTPException(status_code=404, detail="Treasury account not found")
    ...
    if repayment.credit_to_vendor_id:
        credit_vendor = await db.vendors.find_one(
            {"vendor_id": repayment.credit_to_vendor_id}, {"_id": 0}
        )
    ...
    now = datetime.now(timezone.utc)
    payment_date = (
        repayment.payment_date if repayment.payment_date else now.isoformat()[:10]
    )
    ...
    if repayment.currency != loan["currency"]:
        if repayment.amount_in_loan_currency and repayment.amount_in_loan_currency > 0:
            ...
            exchange_rate_used = repayment.exchange_rate or (
                repayment_amount_in_loan_currency / repayment.amount
                if repayment.amount
                else 1.0
            )
            ...
            repayment_amount_in_loan_currency = round(
                repayment.amount * repayment.exchange_rate, 2
            )
            ...
            repayment_amount_in_loan_currency = (
                round(amount_usd / loan_rate, 2) if loan_rate else repayment.amount
            )
            exchange_rate_used = (
                round(repayment_amount_in_loan_currency / repayment.amount, 6)
                if repayment.amount
                else 1.0
            )
    ...
    if credit_vendor:
        ...
        if vendor_commission_rate > 0:
            ...
            vendor_commission_amount = round(
                repayment.amount * vendor_commission_rate / 100, 2
            )
            vendor_commission_base_amount = vendor_commission_amount
...
async def delete_loan(
    request: Request,
    loan_id: str,
    user: dict = Depends(require_permission(Modules.LOANS, Actions.DELETE)),
):
    ...
    if repayment_count > 0:
        raise HTTPException(
            status_code=400,
            detail="Cannot delete loan with repayments. Delete repayments first.",
        )
...
async def get_loans_summary(
    ...
    for loan in loans:
        amount_usd = loan.get(
            "amount_usd", convert_to_usd(loan["amount"], loan["currency"])
        )
        ...
        if loan.get("total_repaid", 0) > loan["amount"]:
            total_interest_earned += convert_to_usd(
                loan["total_repaid"] - loan["amount"], loan["currency"]
            )
    ...
    return {
        ...
        "by_borrower": {
            k: {
                **v,
                "disbursed": round(v["disbursed"], 2),
                "outstanding": round(v["outstanding"], 2),
            }
            for k, v in by_borrower.items()
        },
    }
...
def calculate_debt_interest(
    principal: float, annual_rate: float, days_overdue: int
) -> float:
...
async def get_debts(
    ...
    for debt in debts:
            ...
            try:
                ...
                if now > due_date:
                    ...
                    debt["accrued_interest"] = calculate_debt_interest(
                        outstanding, debt.get("interest_rate", 0), debt["days_overdue"]
                    )
                else:
                    debt["days_overdue"] = 0
        ...
        debt["total_due"] = debt["outstanding_balance"] + debt.get(
            "accrued_interest", 0
        )
...
async def get_debt(
    debt_id: str, user: dict = Depends(require_permission(Modules.DEBTS, Actions.VIEW))
):
    ...
    # Get payment history
    payments = (
        await db.debt_payments.find({"debt_id": debt_id}, {"_id": 0})
        .sort("payment_date", -1)
        .to_list(100)
    )
...
async def create_debt(
    ...
    if debt_data.party_type == DebtPartyType.CLIENT and debt_data.party_id:
        client = await db.clients.find_one(
            {"client_id": debt_data.party_id}, {"_id": 0}
        )
        if client:
            party_name = (
                f"{client.get('first_name', '')} {client.get('last_name', '')}".strip()
        ...
        vendor = await db.vendors.find_one(
            {"vendor_id": debt_data.party_id}, {"_id": 0}
        )
        if vendor:
...
async def update_debt(
    ...
    user: dict = Depends(require_permission(Modules.DEBTS, Actions.EDIT)),
):
    ...
    if "amount" in updates:
        updates["amount_usd"] = convert_to_usd(
            updates["amount"], debt.get("currency", "USD")
        )
...
async def delete_debt(
    request: Request,
    debt_id: str,
    user: dict = Depends(require_permission(Modules.DEBTS, Actions.DELETE)),
):
    ...
    if debt.get("total_paid", 0) > 0:
        raise HTTPException(
            status_code=400,
            detail="Cannot delete debt with payments. Mark as cancelled instead.",
        )
...
async def record_debt_payment(
    ...
    # Verify treasury account
    treasury = await db.treasury_accounts.find_one(
        {"account_id": payment_data.treasury_account_id}, {"_id": 0}
    )
    if not treasury:
        raise HTTPException(status_code=404, detail="Treasury account not found")
    ...
    if debt["debt_type"] == DebtType.RECEIVABLE:
        ...
        income_entry = {
            ...
            "currency": "USD",
            "date": (
                payment_date.split("T")[0] if "T" in str(payment_date) else payment_date
            ),
...
async def get_debts_summary(
    ...
    for debt in debts:
        ...
        amount_usd = debt.get(
            "amount_usd",
            convert_to_usd(debt.get("amount", 0), debt.get("currency", "USD")),
        )
        ...
        if due_date_str and outstanding > 0:
            try:
                ...
                if now > due_date:
                    ...
                    # Calculate interest
                    interest = calculate_debt_interest(
                        outstanding, debt.get("interest_rate", 0), days_overdue
                    )
    ...
    summary["net_position"] = (
        summary["receivables"]["outstanding"] - summary["payables"]["outstanding"]
    )
...
async def get_vendor_commissions_report(
    ...
    for tx in transactions:
        ...
        if currency not in vendor_commissions[vid]["commission_by_currency"]:
            vendor_commissions[vid]["commission_by_currency"][currency] = {
                "base": 0,
                "usd": 0,
            }
    ...
    return {
        ...
        "total_commission_usd": sum(
            v["total_commission_usd"] for v in vendor_commissions.values()
        ),
...
async def get_client_balances_report(
    ...
    for item in tx_data:
        ...
        if currency not in client_tx_map[cid]["by_currency"]:
            client_tx_map[cid]["by_currency"][currency] = {
                "deposits": 0,
                "withdrawals": 0,
            }
        ...
        if tx_type == "deposit":
            ...
            client_tx_map[cid]["by_currency"][currency]["deposits"] += item[
                "total_base"
            ]
        elif tx_type == "withdrawal":
            client_tx_map[cid]["withdrawals_usd"] += item["total_usd"]
            client_tx_map[cid]["withdrawal_count"] += item["count"]
            client_tx_map[cid]["by_currency"][currency]["withdrawals"] += item[
                "total_base"
            ]
        ...
        results.append(
            {
                ...
                "transaction_count": tx_info["deposit_count"]
                + tx_info["withdrawal_count"],
...
async def get_treasury_summary_report(
    ...
    transfers = (
        await db.treasury_transactions.find(
...
async def get_financial_summary_report(
    ...
    total_loan_outstanding = sum(
        l.get("outstanding_balance", l.get("amount", 0)) for l in loans
    )
    ...
    total_treasury_usd = sum(
        convert_to_usd(a.get("balance", 0), a.get("currency", "USD")) for a in accounts
    )
    ...
    return {
        ...
        "vendor_commissions": {"total_paid": total_vendor_commission},
    }
...
async def upload_bank_statement(
    ...
    try:
        if filename.endswith(".csv"):
            ...
            raise HTTPException(
                status_code=400,
                detail="Unsupported file format. Use CSV, Excel, or PDF.",
            )
    except Exception as e:
    ...
    for idx, row in enumerate(parsed_rows):
        ...
        for key, value in row.items():
            ...
            if any(
                x in key_lower for x in ["amount", "value", "debit", "credit", "sum"]
            ):
                try:
                    ...
                    amount = (
                        float(clean_val) if clean_val and clean_val != "None" else None
                    )
                except:
                    pass
                ...
                x in key_lower for x in ["reference", "ref", "txn", "transaction", "id"]
            ):
                reference = str(value) if value else None
            elif any(
                x in key_lower
                for x in ["description", "desc", "narration", "details", "memo"]
            ):
        ...
        if amount is not None:
            ...
            potential_match = await db.treasury_transactions.find_one(
                match_query, {"_id": 0}
            )
            ...
            if potential_match:
                ...
                if abs(variance) < 0.01:
                    ...
                    entry_doc["matched_transaction_id"] = potential_match[
    ...
    await log_activity(
        request, user, "create", "reconciliation", "Uploaded bank statement"
    )
...
async def get_reconciliation_batches(
    ...
    batches = (
        await db.reconciliation_batches.find(query, {"_id": 0})
        .sort("created_at", -1)
        .to_list(limit)
    )
    return batches
...
async def get_reconciliation_batch(
    ...
    entries = await db.reconciliation_entries.find(
...
async def manually_match_entry(
    ...
    tx = await db.treasury_transactions.find_one(
        {"treasury_transaction_id": transaction_id}, {"_id": 0}
    )
    if not tx:
        raise HTTPException(status_code=404, detail="Transaction not found")
    ...
    # Update batch counts
    batch = await db.reconciliation_batches.find_one(
        {"batch_id": entry["batch_id"]}, {"_id": 0}
    )
    if batch:
        await db.reconciliation_batches.update_one(
            {"batch_id": entry["batch_id"]}, {"$inc": {"matched": 1, "unmatched": -1}}
        )
...
async def ignore_reconciliation_entry(
    entry_id: str,
    reason: str = "",
    user: dict = Depends(require_permission(Modules.RECONCILIATION, Actions.EDIT)),
):
...
async def get_psp_reconciliation_details(
    psp_id: str,
    user: dict = Depends(require_permission(Modules.RECONCILIATION, Actions.VIEW)),
):
    ...
    txs = (
        await db.transactions.find(
            {"destination_type": "psp", "psp_id": psp_id, "settled": True}, {"_id": 0}
        )
        .sort("settled_at", -1)
        .to_list(1000)
    )
...
async def get_client_reconciliation(
    ...
    for client in clients:
        ...
        txs = await db.transactions.find(
            {"client_id": client_id, "status": {"$in": ["approved", "completed"]}},
            {"_id": 0},
        ).to_list(10000)
...
async def get_client_reconciliation_details(
    client_id: str,
    user: dict = Depends(require_permission(Modules.RECONCILIATION, Actions.VIEW)),
):
...
async def get_vendor_reconciliation(
    ...
    for vendor in vendors:
        ...
        txs = await db.transactions.find(
            {"vendor_id": vendor_id, "status": {"$in": ["approved", "completed"]}},
            {"_id": 0},
        ).to_list(10000)
...
async def get_reconciliation_summary(
    ...
    bank_batches = (
        await db.reconciliation_batches.find({"type": "bank"}, {"_id": 0})
        .sort("created_at", -1)
        .to_list(10)
    )
    ...
    psp_with_variance = sum(
        1 for p in psp_recon if abs(p.get("total_variance", 0)) > 0.01
    )

    # Client reconciliation
    client_recon = await get_client_reconciliation(user)
    clients_with_discrepancy = sum(
        1 for c in client_recon if c["status"] == "discrepancy"
    )
    total_client_variance = sum(abs(c["variance"]) for c in client_recon)

    # Vendor reconciliation
    vendor_recon = await get_vendor_reconciliation(user)
    vendors_with_discrepancy = sum(
        1 for v in vendor_recon if v["status"] == "discrepancy"
    )
    ...
    return {
        "bank": {
            ...
            "status": (
                "attention"
                if total_unmatched_bank > 0 or total_discrepancies_bank > 0
                else "ok"
            ),
        },
        "psp": {
...
async def get_dates_with_transactions(
    user: dict = Depends(require_permission(Modules.RECONCILIATION, Actions.VIEW))
):
    ...
    for rec in recon_records:
        ...
        if date not in status_map:
            status_map[date] = {"byType": {}, "overall": None}
...
async def get_reconciliation_daily_summary(
    ...
    recon_records = await db.reconciliations.find({"date": date}, {"_id": 0}).to_list(
        100
    )
...
async def get_pending_reconciliation(
    ...
    for r in recon_records:
        if r.get("status") == "completed":
            reconciled_keys.add(
                f"{r.get('date')}_{r.get('account_type')}_{r.get('account_id')}"
            )
    ...
    if not account_type or account_type == "treasury":
        ...
        treasury_txs = await db.treasury_transactions.aggregate(tx_pipeline).to_list(
            5000
        )
    ...
    if not account_type or account_type == "psp":
        psps = await db.psps.find({}, {"_id": 0, "psp_id": 1, "psp_name": 1}).to_list(
            100
        )
        psp_map = {p["psp_id"]: p for p in psps}
    ...
    if not account_type or account_type == "exchanger":
        ...
        vendor_pipeline = [
            ...
            {
                "$group": {
                    "_id": {"date": "$date", "vendor_id": "$vendor_id"},
                    "count": {"$sum": 1},
                    "total_amount": {"$sum": "$amount"},
                }
            },
...
async def submit_reconciliation(
    ...
    await log_activity(
        ...
        "create",
        "reconciliation",
        f"Submitted reconciliation for {data.get('date')}",
    )
...
async def get_calendar_reconciliation_history(
    ...
    records = (
        await db.reconciliations.find({}, {"_id": 0})
        .sort("created_at", -1)
        .to_list(limit)
    )
    return records
...
async def get_calendar_status(
    ...
    for recon in reconciliations:
        ...
        if date not in status_by_date:
            status_by_date[date] = {"byType": {}, "overall": None}
    ...
    return {"month": f"{year}-{month_num:02d}", "status": status_by_date}


# ============== MESSAGING & CHANNELS ==============
# Endpoints live in chat.py and are included here.
...
async def get_daily_reconciliation(
    ...
    recon_items = await db.reconciliation_items.find(
        {"date": {"$gte": today.isoformat(), "$lt": tomorrow.isoformat()}}, {"_id": 0}
    ).to_list(1000)
    ...
    flagged_ids = {
        item["reference_id"] for item in recon_items if item.get("status") == "flagged"
    }
    ...
    for tx in transactions:
        ...
        status = (
            "reconciled"
            if tx_id in reconciled_ids
            else ("flagged" if tx_id in flagged_ids else "pending")
        )
        pending_items.append(
            {
                "id": tx_id,
    ...
    for ie in ie_entries:
        ...
        status = (
            "reconciled"
            if ie_id in reconciled_ids
            else ("flagged" if ie_id in flagged_ids else "pending")
        )
        pending_items.append(
            {
                "id": ie_id,
    ...
    for ttx in treasury_txs:
        ...
        status = (
            "reconciled"
            if ttx_id in reconciled_ids
            else ("flagged" if ttx_id in flagged_ids else "pending")
        )
        pending_items.append(
            {
                "id": ttx_id,
    ...
    return {
        ...
        "stats": {
            ...
            "pending": pending,
            "flagged": flagged,
            "reconciled_percent": round(
                (reconciled / total * 100) if total > 0 else 100, 1
            ),
        },
    }
...
async def bulk_reconcile(
    ...
    for item in items:
        ...
        existing = await db.reconciliation_items.find_one(
            {"reference_id": reference_id}
        )
    ...
    return {
        "message": f"Reconciled {reconciled_count} items",
...
async def create_adjustment(
    ...
    if treasury_account_id:
        ...
        await db.treasury_accounts.update_one(
            {"account_id": treasury_account_id}, update_op
        )
...
async def get_reconciliation_audit_trail(
    ...
    history = (
        await db.reconciliations.find(query, {"_id": 0})
        .sort("created_at", -1)
        .to_list(limit)
    )
    return history
...
async def export_reconciliation_audit_trail(
    ...
    if format == "pdf":
        ...
        buffer = BytesIO()
        doc = SimpleDocTemplate(
            buffer, pagesize=landscape(letter), topMargin=30, bottomMargin=30
        )
        elements = []
        styles = getSampleStyleSheet()
        ...
        elements.append(
            Paragraph(
                f"Total Records: {total} | Completed: {completed} | Pending: {pending}",
                styles["Normal"],
            )
        )
        ...
        return Response(
            ...
            media_type="application/pdf",
            headers={
                "Content-Disposition": f"attachment; filename=reconciliation_history_{datetime.now().strftime('%Y%m%d')}.pdf"
            },
        )
        ...
        return Response(
            ...
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={
                "Content-Disposition": f"attachment; filename=reconciliation_history_{datetime.now().strftime('%Y%m%d')}.xlsx"
            },
        )
...
async def get_flagged_items(
    ...
    items = (
        await db.reconciliation_items.find({"status": "flagged"}, {"_id": 0})
        .sort("flagged_at", -1)
        .to_list(1000)
    )
    return items
...
async def write_off_variance(
    ...
    return {
        "message": "Variance written off successfully",
        "reference_id": reference_id,
    }
...
async def get_roles(
    user: dict = Depends(require_permission(Modules.ROLES, Actions.VIEW))
):
    """Get all roles"""
...
async def get_role(
    role_id: str, user: dict = Depends(require_permission(Modules.ROLES, Actions.VIEW))
):
    """Get a specific role"""
...
async def create_role(
    ...
    user: dict = Depends(require_permission(Modules.ROLES, Actions.CREATE)),
):
    """Create a new role"""
...
async def update_role(
    ...
    user: dict = Depends(require_permission(Modules.ROLES, Actions.EDIT)),
):
    """Update a role"""
    role = await db.roles.find_one({"role_id": role_id}, {"_id": 0})
    if not role:
...
async def delete_role(
    request: Request,
    role_id: str,
    user: dict = Depends(require_permission(Modules.ROLES, Actions.DELETE)),
):
...
async def get_modules_and_actions(
    user: dict = Depends(require_permission(Modules.ROLES, Actions.VIEW))
):
    """Get all available modules and actions for permission configuration"""
    return {
        "modules": [
            {"id": m, "name": MODULE_DISPLAY_NAMES.get(m, m)} for m in ALL_MODULES
        ],
        "actions": ALL_ACTIONS,
    }
...
async def get_my_permissions(user: dict = Depends(get_current_user)):
    ...
    role = await db.roles.find_one(
        {"$or": [{"role_id": role_id}, {"name": role_id}]}, {"_id": 0}
    )
...
async def send_approval_notification(notification_type: str, details: dict):
    ...
    try:
        ...
        roles_with_approve = await db.roles.find(
            {"permissions.approvals": {"$exists": True}},
        ...
        approvers = await db.users.find(
            {
                "$or": [{"role": {"$in": role_ids}}, {"role_id": {"$in": role_ids}}],
                "is_active": {"$ne": False},
            },
        ...
        to_emails = [
            u["email"]
            for u in approvers
            if u.get("email") and u["user_id"] not in opted_out
        ]
        if not to_emails:
            return
        ...
        logger.info(
            f"Approval notification sent to {len(to_emails)} approvers for {notification_type}"
        )
    except Exception as e:
        logger.error(f"Failed to send approval notification: {e}")


async def send_exchanger_notification(
    notification_type: str, vendor_id: str, details: dict
):
    """Send email notification to a specific exchanger"""
    try:
        vendor = await db.vendors.find_one(
            {"vendor_id": vendor_id}, {"_id": 0, "user_id": 1, "vendor_name": 1}
        )
        if not vendor or not vendor.get("user_id"):
            return

        user_doc = await db.users.find_one(
            {"user_id": vendor["user_id"]}, {"_id": 0, "email": 1, "user_id": 1}
        )
        ...
        logger.info(
            f"Exchanger notification sent to {vendor_name} for {notification_type}"
        )
    except Exception as e:
...
async def get_email_settings(
    ...
    return {
        ...
        "smtp_from_email": settings.get(
            "smtp_from_email", settings.get("smtp_email", "")
        ),
...
async def test_email_settings(
    ...
    if (
        not settings
        or not settings.get("smtp_email")
        or not settings.get("smtp_password")
    ):
...
async def send_daily_report_now(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to send daily report: {str(e)}"
        )
...
async def generate_daily_report_html():
    ...
    today_txs = await db.transactions.find(
        {"created_at": {"$gte": today_start.isoformat()}}, {"_id": 0}
    ).to_list(10000)

    today_deposits = sum(
        t.get("amount", 0) for t in today_txs if t.get("transaction_type") == "deposit"
    )
    ...
    total_treasury = sum(
        convert_to_usd(a.get("balance", 0), a.get("currency", "USD"))
    ...
    psp_pending_txs = await db.transactions.find(
        {"destination_type": "psp", "settled": {"$ne": True}}, {"_id": 0}
    ).to_list(1000)

    # Outstanding accounts
    debts = await db.debts.find({"status": {"$ne": "fully_paid"}}, {"_id": 0}).to_list(
        1000
    )
    total_receivables = sum(
        d.get("amount", 0) - d.get("paid_amount", 0)
        for d in debts
        if d.get("debt_type") == "receivable"
    )
    total_payables = sum(
        d.get("amount", 0) - d.get("paid_amount", 0)
        for d in debts
    ...
    today_disbursements = [
        lt for lt in today_loan_txs if lt.get("transaction_type") == "disbursement"
    ]
    today_repayments = [
        lt for lt in today_loan_txs if lt.get("transaction_type") == "repayment"
    ]
    ...
    if dealing_pnl_record:
        ...
        prev_record = await db.dealing_pnl.find_one(
            {"date": {"$lt": today_date}}, {"_id": 0}, sort=[("date", -1)]
        )
    ...
    total_recon_discrepancies = sum(
        b.get("discrepancies", 0) for b in today_recon_batches
    )
...
async def generate_monthly_report_html(year: int = None, month: int = None):
    ...
    month_txs = await db.transactions.find(
        {"created_at": {"$gte": month_start_iso, "$lte": month_end_iso}}, {"_id": 0}
    ).to_list(50000)

    approved_txs = [
        t for t in month_txs if t.get("status") in ["approved", "completed"]
    ]
    total_deposits = sum(
        t.get("amount", 0)
    ...
    ie_income = sum(
        e.get("amount_usd", e.get("amount", 0))
        for e in month_ie
        if e.get("entry_type") == "income"
    )
    ie_expense = sum(
    ...
    active_loans = await db.loans.find(
        {"status": {"$in": ["active", "partially_paid"]}}, {"_id": 0}
    ).to_list(1000)
    ...
    ie_commission_total = sum(
        e.get("vendor_commission_amount", 0) or 0
        for e in month_ie
        if e.get("vendor_id")
    )
    ...
    for v in vendors:
        ...
        v_deps = sum(
            t.get("amount", 0) for t in v_txs if t.get("transaction_type") == "deposit"
        )
        v_wds = sum(
            t.get("amount", 0)
            for t in v_txs
    ...
    total_settled = sum(
        s.get("settlement_amount", s.get("amount", 0)) for s in month_settlements
    )
    ...
    recon_total_matched = sum(b.get("matched", 0) for b in month_recon_batches) + sum(
        r.get("matched_count", 0) for r in month_recon_history
    )
    recon_total_unmatched = sum(b.get("unmatched", 0) for b in month_recon_batches)
    recon_total_flagged = sum(
        b.get("discrepancies", 0) for b in month_recon_batches
    ) + sum(r.get("flagged_count", 0) for r in month_recon_history)
...
async def run_audit_checks() -> dict:
    ...
    for tx in all_txs:
        ...
        if (
            ...
            and tx.get("transaction_type") == "deposit"
            and not tx.get("proof_image")
            and not tx.get("accountant_proof_image")
        ):
    ...
    treasury_accounts = {
        a["account_id"]: a
        for a in await db.treasury_accounts.find({}, {"_id": 0}).to_list(100)
    }
    ...
    for tx in psp_txs:
        ...
        if tx.get("settled"):
            ...
            if dest_acct and dest_acct.get("currency", "USD") != tx.get(
                "currency", "USD"
            ):
    ...
    anomaly_settings = (
        await db.app_settings.find_one({"setting_type": "audit"}, {"_id": 0}) or {}
    )
    ...
    for acct_id, acct in treasury_accounts.items():
        ...
        ttxs = await db.treasury_transactions.find(
            {"account_id": acct_id}, {"_id": 0}
        ).to_list(50000)
    ...
    stats["passed"] = max(
        0, stats["total_checks"] - stats["critical"] - stats["warning"]
    )
...
async def run_audit_scan(
    request: Request,
    user: dict = Depends(require_permission(Modules.AUDIT, Actions.CREATE)),
):
...
async def get_latest_audit(
    user: dict = Depends(require_permission(Modules.AUDIT, Actions.VIEW))
):
...
async def get_audit_history(
    ...
    scans = (
        await db.audit_scans.find({}, {"_id": 0, "findings": 0})
        .sort("scanned_at", -1)
        .to_list(limit)
    )
    return scans
...
async def update_audit_settings(
    ...
    user: dict = Depends(require_permission(Modules.AUDIT, Actions.EDIT)),
):
    ...
    await db.app_settings.update_one(
        {"setting_type": "audit"}, {"$set": updates}, upsert=True
    )
...
async def send_audit_alert_email(result: dict):
    ...
    try:
        audit_settings = await db.app_settings.find_one(
            {"setting_type": "audit"}, {"_id": 0}
        )
...
async def get_fx_rates_endpoint(user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "fetched_at": (
            _fx_cache.get("fetched_at", "").isoformat()
            if _fx_cache.get("fetched_at")
            else None
        ),
...
async def refresh_fx_rates(
    ...
    return {
        ...
        "sample_rates": {
            k: rates.get(k) for k in ["USD", "EUR", "GBP", "AED", "INR"] if k in rates
        },
    }


@api_router.get("/settings/manual-fx-rates")
async def get_manual_fx_rates(
    user: dict = Depends(require_permission(Modules.SETTINGS, Actions.VIEW))
):
...
async def convert_currency_endpoint(
    ...
    if from_rate is None or to_rate is None:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported currency: {from_currency if from_rate is None else to_currency}",
        )
...
async def get_all_logs(
    ...
    logs = (
        await db.system_logs.find(query, {"_id": 0})
        .sort("timestamp", -1)
        .skip(skip_n)
        .limit(page_size)
        .to_list(page_size)
...
async def get_logs_stats(
    ...
    failed_logins = await db.system_logs.count_documents(
        {"log_type": "auth", "action": "login_failed", "timestamp": {"$gte": week_ago}}
    )
    ...
    return {
        ...
        "active_users": [
            {"user": u["_id"], "count": u["count"]} for u in active_users if u["_id"]
        ],
        "common_actions": [
...
async def start_impersonation(
    ...
    impersonation_token = create_jwt_token(
        target["user_id"], target["email"], target["role"]
    )
...
async def get_impersonation_logs(
    ...
    logs = (
        await db.impersonation_logs.find({}, {"_id": 0})
        .sort("login_time", -1)
        .to_list(limit)
    )
    return logs
